...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_794_688);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_BYTES).toBe(11_946_002);
    expect(MAX_PACKED_BYTES).toBe(11_941_906 + 4_096);
    expect(budget).toContain("11,941,906 + 4,096 = 11,946,002");
    ...
    expect(MAX_PACKED_ENTRIES).toBe(588);
    expect(MAX_PACKED_FILES).toBe(588);
    ...
    expect(budget).toContain("23,191,418 + 353 + 65 = 23,191,836");
    expect(budget).toContain("7826b2c9bddd12b5d982eda11ca114b7e98d7d4f649d1984490272241b31afbd");
    expect(MAX_UNPACKED_BYTES).toBe(23_191_836);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_191_771 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      entryCount: { min: 588, max: 588 },
      fileCount: { min: 588, max: 588 },
      packedBytes: { min: 1_600_000, max: 11_946_002 },
      unpackedBytes: { min: 9_000_000, max: 23_191_836 },
    });
  });
