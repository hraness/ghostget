...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_653_376);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_BYTES).toBe(11_927_223);
    expect(MAX_PACKED_BYTES).toBe(11_910_740 + 12_387 + 4_096);
    ...
    expect(MAX_PACKED_FILES).toBe(578);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 11_927_223 },
      unpackedBytes: { min: 9_000_000, max: 23_060_613 },
    });
  });
