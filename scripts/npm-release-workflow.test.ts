...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(24_040_960);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_993_873, unpackedBytes: 23_429_566, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_010_356);
    expect(MAX_PACKED_BYTES).toBe(11_993_873 + 12_387 + 4_096);
    expect(budget).toContain("11,993,645 + 12,387 + 4,096 = 12,010,128");
    expect(budget).toContain("23,427,470 + 353 + 65 = 23,427,888 unpacked");
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_429_984);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_429_566 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 12_010_356 },
      unpackedBytes: { min: 9_000_000, max: 23_429_984 },
    });
  });
