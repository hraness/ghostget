...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(24_017_920);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_987_773, unpackedBytes: 23_406_537, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_004_256);
    expect(MAX_PACKED_BYTES).toBe(11_987_773 + 12_387 + 4_096);
    expect(budget).toContain("11,997,841 + 12,387 + 4,096 = 12,014,324 packed");
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_406_955);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_406_537 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 12_004_256 },
      unpackedBytes: { min: 9_000_000, max: 23_406_955 },
    });
  });
