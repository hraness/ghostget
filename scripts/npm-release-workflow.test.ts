...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_099_392);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_UNPACKED_BYTES).toBe(22_527_197);
    expect(MAX_UNPACKED_BYTES).toBe(22_523_437 + 3_695 + 65);
    expect(budget).toContain("22,523,437 + 3,695 + 65 = 22,527,197");
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      unpackedBytes: { min: 9_000_000, max: 22_527_197 },
    });
  });
