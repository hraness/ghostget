...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_051_264);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_BYTES).toBe(11_653_911);
    expect(MAX_PACKED_BYTES).toBe(11_649_815 + 4_096);
    expect(MAX_PACKED_ENTRIES).toBe(558);
    expect(MAX_PACKED_FILES).toBe(558);
    expect(MAX_UNPACKED_BYTES).toBe(22_479_348);
    expect(MAX_UNPACKED_BYTES).toBe(22_479_283 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 11_653_911 },
      unpackedBytes: { min: 9_000_000, max: 22_479_348 },
    });
  });
