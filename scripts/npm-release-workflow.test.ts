...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(24_017_408);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_987_539, unpackedBytes: 23_405_856, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_004_022);
    expect(MAX_PACKED_BYTES).toBe(11_987_539 + 12_387 + 4_096);
    expect(budget).toContain("11,986,281 + 12,387 + 4,096 =");
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_406_274);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_405_856 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 12_004_022 },
      unpackedBytes: { min: 9_000_000, max: 23_406_274 },
    });
  });
