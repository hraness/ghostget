...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_261_184);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_ENTRIES).toBe(565);
    expect(MAX_PACKED_FILES).toBe(565);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(22_682_119);
    expect(MAX_UNPACKED_BYTES).toBe(22_682_054 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      entryCount: { min: 565, max: 565 },
      fileCount: { min: 565, max: 565 },
      packedBytes: { min: 1_600_000, max: 11_693_939 },
      unpackedBytes: { min: 9_000_000, max: 22_682_119 },
    });
  });
