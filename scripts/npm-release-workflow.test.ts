...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_952_896);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_983_797, unpackedBytes: 23_341_689, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_000_280);
    expect(MAX_PACKED_BYTES).toBe(11_983_797 + 12_387 + 4_096);
    ...
    expect(budget).toContain("23,340,769 + 353 + 65 = 23,341,187");
    expect(budget).toContain(
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_342_107);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_341_689 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 12_000_280 },
      unpackedBytes: { min: 9_000_000, max: 23_342_107 },
    });
  });
