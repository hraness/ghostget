...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_857_664);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_954_952, unpackedBytes: 23_251_469, entryCount: 591,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(11_971_435);
    expect(MAX_PACKED_BYTES).toBe(11_954_952 + 12_387 + 4_096);
    expect(budget).toContain("11,954,952 + 12,387 + 4,096 = 11,971,435");
    expect(budget).toContain("23,251,469 + 353 + 65 = 23,251,887");
    expect(budget).toContain(
      "bff4d929cac0d41f56d8d67b003c64f582a80d3a9773a69568d3fb2912a7b8ea",
    );
    ...
    expect(MAX_PACKED_ENTRIES).toBe(591);
    expect(MAX_PACKED_FILES).toBe(591);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_251_887);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_251_469 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 11_971_435 },
      unpackedBytes: { min: 9_000_000, max: 23_251_887 },
    });
  });
