...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_958_528);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_972_740, unpackedBytes: 23_347_074, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(11_989_223);
    expect(MAX_PACKED_BYTES).toBe(11_972_740 + 12_387 + 4_096);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_347_492);
    ...
    expect(budget).toContain("72f1c95883b4541752396bdd89d0c370839c43aa365b42cd17ea01579b661f9b");
    expect(MAX_UNPACKED_BYTES).toBe(23_347_074 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 11_989_223 },
      unpackedBytes: { min: 9_000_000, max: 23_347_492 },
    });
  });
