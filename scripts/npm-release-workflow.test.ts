...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_100_416);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_UNPACKED_BYTES).toBe(22_528_345);
    expect(MAX_UNPACKED_BYTES).toBe(22_520_157 + 8_123 + 65);
    expect(budget).toContain("22,520,157 + 8,123 + 65 = 22,528,345");
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      unpackedBytes: { min: 9_000_000, max: 22_528_345 },
    });
  });
