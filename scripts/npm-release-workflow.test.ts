...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(24_059_392);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 11_998_599, unpackedBytes: 23_447_811, entryCount: 596,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_015_082);
    expect(MAX_PACKED_BYTES).toBe(11_998_599 + 12_387 + 4_096);
    expect(budget).toContain("11,998,420 + 12,387 + 4,096 = 12,014,903");
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_448_229);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_447_811 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 12_015_082 },
      unpackedBytes: { min: 9_000_000, max: 23_448_229 },
    });
  });
