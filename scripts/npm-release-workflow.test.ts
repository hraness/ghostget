...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_091_200);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_UNPACKED_BYTES).toBe(22_519_270);
    expect(MAX_UNPACKED_BYTES).toBe(22_517_657 + 1_548 + 65);
    expect(budget).toContain("22,517,657 + 1,548 + 65 = 22,519,270");
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      unpackedBytes: { min: 9_000_000, max: 22_519_270 },
    });
  });
