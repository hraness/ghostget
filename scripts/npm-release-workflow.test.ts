...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(24_214_528);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(repairPackageMeasurement).toMatchObject({
      ...
      packedBytes: 12_050_663, unpackedBytes: 23_598_059, entryCount: 601,
      packedPlatformProjection: 12_387, packedPortabilityAllowance: 4_096,
      payloadPlatformProjection: 353, payloadAllowance: 65,
    });
    expect(MAX_PACKED_BYTES).toBe(12_067_146);
    expect(MAX_PACKED_BYTES).toBe(12_050_663 + 12_387 + 4_096);
    ...
    expect(MAX_PACKED_ENTRIES).toBe(601);
    expect(MAX_PACKED_FILES).toBe(601);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_598_477);
    ...
    expect(MAX_UNPACKED_BYTES).toBe(23_598_059 + 353 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      entryCount: { min: 601, max: 601 },
      fileCount: { min: 601, max: 601 },
      packedBytes: { min: 1_600_000, max: 12_067_146 },
      unpackedBytes: { min: 9_000_000, max: 23_598_477 },
    });
  });
