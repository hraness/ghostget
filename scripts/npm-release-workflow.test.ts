...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(23_069_184);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_BYTES).toBe(11_658_570);
    expect(MAX_PACKED_BYTES).toBe(11_654_474 + 4_096);
    expect(MAX_PACKED_ENTRIES).toBe(558);
    expect(MAX_PACKED_FILES).toBe(558);
    expect(MAX_UNPACKED_BYTES).toBe(22_497_378);
    expect(MAX_UNPACKED_BYTES).toBe(22_497_313 + 65);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 11_658_570 },
      unpackedBytes: { min: 9_000_000, max: 22_497_378 },
    });
  });
