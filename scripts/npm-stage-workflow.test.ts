...
describe("npm publication contract", () => {
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(MAX_PACKED_BYTES).toBe(2_251_305);
    expect(MAX_PACKED_ENTRIES).toBe(501);
    expect(MAX_PACKED_FILES).toBe(501);
    expect(MAX_UNPACKED_BYTES).toBe(12_413_003);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      packedBytes: { min: 1_600_000, max: 2_251_305 },
      unpackedBytes: { min: 9_000_000, max: 12_413_003 },
    });
  });
