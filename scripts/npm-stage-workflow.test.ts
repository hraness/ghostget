...
describe("npm publication contract", () => {
  test("derives the tar expansion ceiling from the reviewed package budget", async () => {
    ...
    expect(MAX_PACKAGE_TAR_BYTES).toBe(12_678_144);
  ...
  test("keeps one narrow release-authoritative package budget", async () => {
    ...
    expect(budget).toContain("one `bun pm pack` artifact");
    expect(budget).toContain("2,071,730 packed bytes");
    expect(budget).toContain("12,192,284 unpacked bytes, and");
    expect(budget).toContain("466 files");
    expect(budget).toContain(
      "6acd0d96e45604d3fb4953ca7f6dd14a7b45b28a4b39edf186f71933f6309c00",
    );
    ...
    expect(budget).toContain("7,716 unpacked bytes of bounded headroom");
    expect(MAX_PACKED_BYTES).toBe(2_178_192);
    expect(MAX_PACKED_ENTRIES).toBe(466);
    expect(MAX_PACKED_FILES).toBe(466);
    expect(MAX_UNPACKED_BYTES).toBe(12_200_000);
    ...
    expect(packageArtifactBudget).toEqual({
      ...
      unpackedBytes: { min: 9_000_000, max: 12_200_000 },
    });
  });
