 ...
 *
 * Only operations whose observable contract survives the engine boundary
 * belong behind it. In particular the sidecar reports a missing path as a
 * `CustodyError` domain failure, never an `ENOENT` `ErrnoException`, so
 * custody checks whose callers branch on `error.code === "ENOENT"` keep the
 * direct `@hraness/local-custody` imports instead of routing through this
 * engine.
 */
