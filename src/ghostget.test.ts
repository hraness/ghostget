...
import { canonicalJson, isWebSessionOperation, sha256, type GhostgetManifest } from "./model";
...
function xThreadManifest(): GhostgetManifest {
...
function bundledWebManifest(site: "bluesky" | "linkedin" | "x"): GhostgetManifest {
  return JSON.parse(readFileSync(
...
function reviewedTemplateReservationManifest(): GhostgetManifest {
  return {
    schemaVersion: 5,
    id: "example-api",
    version: "1.0.0",
...
): GhostgetClipEnvironmentInspection {
  const report = {
...
describe("run reconciliation CLI", () => {
  test("passes exact optional input to the read-only reconciler and returns its distinct exit status", async () => {
    ...
    try {
      ...
      const inconclusiveGhostget = capture();
      const inconclusiveCode = await main(
        ["runs", "reconcile", runId, "--json"],
        testState.environment,
        inconclusiveGhostget.output,
        {
      ...
      expect(inconclusiveGhostget.stderr()).toBe("");
      expect(JSON.parse(inconclusiveGhostget.stdout())).toEqual(reconciliationResult(false, runId));
    } finally {
  ...
  test("rejects malformed reconciliation input before calling the reconciler", async () => {
    ...
    try {
      ...
      expect(wrench.stderr()).toContain("ghostget:");
    } finally {
      rmSync(testState.directory, { recursive: true, force: true });
    }
  });
...
describe("auth CLI", () => {
  ...
  test("onboards, binds, and explicitly syncs a distinct WhatsApp linked device", async () => {
    ...
    try {
      ...
      expect(added.stdout()).toContain("ghostget auth pair whatsapp-protocol");
  ...
  test("guides lifecycle-free Beeper stores to subject binding instead of pairing", async () => {
    ...
    try {
      ...
      expect(added.stdout()).toContain(
        "ghostget auth bind beeper-local --site beeper",
      );
      expect(added.stdout()).not.toContain("ghostget auth pair beeper-local");
    } finally {
      rmSync(testState.directory, { recursive: true, force: true });
    }
  });
  ...
  test("runs the bounded Beeper Message Like Me export through the private runtime", async () => {
    ...
    try {
      ...
      expect(code).toBe(0);
      expect(wrench.stderr()).toBe([
        "ghostget: Beeper export: checking prior private export state",
        "ghostget: Beeper export: private recovery complete; 0 directories reclaimed, 0 published bundles preserved",
        "ghostget: Beeper export: preparing pinned official CLI",
        "ghostget: Beeper export: discovering accounts; 30s elapsed",
        "ghostget: Beeper export: 3 accounts discovered",
        "ghostget: Beeper export: account 1/3 started",
        "ghostget: Beeper export: account 1/3 running; 30s elapsed",
        "ghostget: Beeper export: account 1/3 validating; 0s elapsed",
        "ghostget: Beeper export: account 1/3 complete; 4 chats, 50 messages total",
        "ghostget: Beeper export: account 2/3 skipped; chat limit reached",
        "ghostget: Beeper export: verifying 3 connected accounts",
        "ghostget: Beeper export: verifying connected accounts; 60s elapsed",
        "ghostget: Beeper export: converting 3 accounts; 10 chats, 500 messages total",
        "ghostget: Beeper export: converting local bundle; 30s elapsed",
        "ghostget: Beeper export: building local bundle; 50 records, 4096 bytes; 30s elapsed",
        "ghostget: Beeper export: validating local bundle; 100 records, 8192 bytes; 0s elapsed",
        "ghostget: Beeper export: publishing local bundle atomically; 100 records, 8192 bytes; 0s elapsed",
        "ghostget: Beeper export: removing private raw shards; 0s elapsed",
        "",
      ].join("\n"));
  ...
  test("runs one bound WhatsApp account export with transparent progress and an exact receipt", async () => {
    ...
    try {
      ...
      expect(wrench.stderr()).toBe([
        "ghostget: WhatsApp export: binding the quiescent local store",
        "ghostget: WhatsApp export: page 1 reading; 0 local message rows observed",
        "ghostget: WhatsApp export: page 1 completed; 12 local message rows observed",
        "ghostget: WhatsApp export: local projection complete; 12 rows, 3 conversations, 4 participants",
        "ghostget: WhatsApp export: building private bundle; 20 records, 4096 bytes; 1s elapsed",
        "ghostget: WhatsApp export: publishing bundle v2 artifact 6/6; 20 records converted",
        "",
      ].join("\n"));
      expect(observed).toMatchObject({
        auth: {
  ...
  test("prints the body-free Beeper relationship receipt envelope transparently", async () => {
    ...
    try {
      ...
      expect(wrench.stderr()).toContain(
        "ghostget: Beeper export: deriving body-free relationship summary; 50 records; 30s elapsed",
      );
      expect(wrench.stderr()).toContain(
        "ghostget: Beeper export: body-free summary complete; 2 relationships, 8 interactions from 100 records",
      );
      expect(observed).toMatchObject({
  ...
  test("routes Gmail OAuth reads and clips through the official private capture boundary", async () => {
    ...
    try {
      ...
      const dependencies: Partial<GhostgetDependencies> = {
        clipMain: () => {
          genericClipCalls += 1;
...
describe("Ghostget media routing", () => {
  test("does not load the media runtime for an unrelated Ghostget command", async () => {
    const wrench = capture();
    let mediaLoads = 0;
    const code = await main(
      ["platforms", "--json"],
  ...
  test("passes translated media arguments, environment, signal, and exit code through unchanged", async () => {
    const environment = { ...process.env, GHOSTGET_MEDIA_HOME: "/tmp/media-library" };
    const wrench = capture();
    const controller = new AbortController();
    const calls: string[][] = [];
  ...
  test("keeps the KB inspection opaque while preserving JSON, terminal rendering, and readiness", async () => {
    ...
    let renderCalls = 0;
    const inspection: GhostgetClipEnvironmentInspection = {
      report,
      renderReport: () => {
        renderCalls += 1;
    ...
    try {
      ...
      expect(JSON.parse(json.stdout())).toMatchObject({
        capture: report,
        ghostget: { browserCaptureBootstrapReady: true },
      });
  ...
  test("adds media evidence to doctor without making media readiness weaken action readiness", async () => {
    const environment = { ...process.env, GHOSTGET_STATE_HOME: state().directory };
    ...
    try {
      ...
      rmSync(environment.GHOSTGET_STATE_HOME, { recursive: true, force: true });
    }
  });
  ...
  test("reports official-provider readiness without requiring browser automation or exposing token metadata", async () => {
    ...
    try {
      ...
      expect(parsed.oh).toEqual(parsed.ghostget);
      expect(parsed.wrench).toEqual(parsed.ghostget);
...
describe("doctor authenticated API readiness", () => {
  test("reports observed LinkedIn private Article readiness independently from X", async () => {
    ...
    try {
      ...
      expect(linkedinReport.ghostget.webSessionSites.find((site) => site.site === "linkedin")).toEqual({
        site: "linkedin",
      ...
      expect(linkedinReport.ghostget.webSessionSites.find((site) => site.site === "x")).toMatchObject({
        site: "x",
        adapters: ["x-web"],
        observedOperations: [
          "articles.draft.save",
      ...
      const readyLinkedIn = readyReport.ghostget.webSessionSites.find((site) => site.site === "linkedin");
      ...
      expect(readyReport.ghostget.webSessionSites.find((site) => site.site === "x")).toMatchObject({
...
describe("provider source plugin discovery CLI", () => {
  ...
  test("renders Ghostget as the program and reports an unknown plugin distinctly", async () => {
    const testState = state();
    try {
      const listed = capture();
      expect(await main(
      ...
      expect(listed.stdout()).toStartWith(
        "Ghostget trusted provider source plugins",
      );
      expect(listed.stdout()).toContain(
        "Ghostget installed portable provider plugins",
      );
      expect(listed.stdout()).toContain("Run 'ghostget plugin show <id>'");
      expect(listed.stdout()).toContain("Run 'ghostget capabilities [adapter]'");
      ...
      expect(missing.stdout()).toBe(
        "Ghostget provider plugin missing-plugin was not found.\n"
...
describe("reviewed platform policy helpers", () => {
  ...
  test("keeps reviewed URL-form OAuth scopes exact in capability JSON", async () => {
    ...
    const manifest = JSON.parse(readFileSync(manifestPath, "utf8")) as GhostgetManifest;
    try {
  ...
  test("uses one active portable catalog for adapter validation, installation, and HAR derivation", async () => {
    ...
    try {
      ...
      expect(doctorReport.ghostget.officialProviders.find(
        (provider) =>
  ...
  test("keeps the archived LinkedIn DOM adapter diagnostic-only while migrating its exact installed hash", async () => {
    ...
    const oldManifest = JSON.parse(readFileSync(oldPath, "utf8")) as GhostgetManifest;
    const newManifest = JSON.parse(readFileSync(newPath, "utf8")) as GhostgetManifest;
    try {
...
describe("CLI previews and exit semantics", () => {
  ...
  test("shows requested actors only for code-owned provider writes and rejects arbitrary browser input", async () => {
    ...
    try {
      ...
      const browserManifest: GhostgetManifest = {
        ...manifest("R2"),
        operations: {
          "messaging.send": {
            ...browserOperation,
      ...
      const dependencies: Partial<GhostgetDependencies> = {
        loadImsgDirectInstallRuntime: async () => ({
          installReviewedImsgBinary: async (
            ...
            observedStateHomes.push(environment.GHOSTGET_STATE_HOME);
          ...
          GhostgetDependencies["loadImsgDirectInstallRuntime"]
        >>),
      };
...
describe("Ghostget process termination boundary", () => {
  test("reserves the browser cleanup window before restoring forced signal delivery", async () => {
    ...
    try {
      const operation = runGhostgetProcess({
