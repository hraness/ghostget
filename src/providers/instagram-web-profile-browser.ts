...
export type InstagramProfileBrowserTransport = {
  ...
  readonly readProfileHtml: (profile: string) => Promise<string>;
...
export async function createInstagramProfileBrowserTransport(
  ...
  const run = async (
    ...
    if (result.authWall === true) {
      if (
        (binding.kind !== "html" && binding.kind !== "profile-html")
        || result.status !== 200
  ...
  return Object.freeze({
    ...
    readProfileHtml: async (profileValue: string) => {
      if (state !== "profile-pending") {
        throw new Error("Instagram profile browser page fallback is out of order");
      }
      const profile = exactInstagramProfile(profileValue);
      const maximumBytes = Math.min(
        MAX_VIEWER_HTML_BYTES,
