  ...
  async check(id:string,digest:string,use?:string):Promise<AgentApprovalResponse> {
    ...
    if(e!==undefined&&e.checked.digest===digest) {
      ...
      if(e.status==="allowed"||e.status==="pending") {
        ...
        // Claim after the await: a concurrent check may have claimed the grant while this one waited.
        else if(e.status==="allowed") {if(!admits(e,use))return expired;e.holder??=use===undefined?randomBytes(32):holderKey(use);}
      }
      status=e.status;
    }
