...
var POLL_FIELDS = "duration_minutes,end_datetime,id,options,voting_status";
var PLACE_FIELDS = "contained_within,country,country_code,full_name,geo,id,name,place_type";
var USER_FIELDS = [
  "affiliation",
  "created_at",
  "description",
...
var CHAT_CONVERSATION_EXPANSIONS = "admin_ids,member_ids,participant_ids";
var CHAT_MESSAGE_EVENT_FIELDS = [
  "conversation_id",
  "conversation_token",
  "created_at_msec",
  "encoded_event",
...
var FILE_VERIFY_CHUNK_BYTES = 1024 * 1024;
var MAX_MEDIA_STATUS_POLLS = 100;
var MAX_MEDIA_STATUS_WAIT_MS = 8 * 60000;
var MAX_CHAT_KEY_EVENTS = 1000;
var MAX_CHAT_ENVELOPE_CHARACTERS = 2 * 1024 * 1024;
function isRecord(value) {
  return typeof value === "object" && value !== null && !Array.isArray(value);
}
function record(value, label) {
  if (!isRecord(value))
    throw new Error(`official X ${label} is not an object`);
  return value;
}
function recordBody(response, label) {
  if (!isRecord(response.body))
    throw new Error(`official X ${label} response is not an object`);
  return response.body;
}
function dataRecord(response, label) {
  const body = recordBody(response, label);
  if (body.errors !== undefined) {
    if (!Array.isArray(body.errors))
      throw new Error(`official X ${label} response returned invalid errors`);
    if (body.errors.length > 0)
      throw new Error(`official X ${label} response returned provider errors`);
  }
  if (!isRecord(body.data))
    throw new Error(`official X ${label} response omitted data`);
  return body.data;
}
...
function responseItems(response, label) {
  const body = recordBody(response, label);
  if (body.data === undefined)
    return [];
  if (!Array.isArray(body.data))
    throw new Error(`official X ${label} response data is not a collection`);
  return body.data;
}
function inputString(context, name) {
  const value = context.input[name];
  if (typeof value !== "string")
    throw new Error(`input.${name} must be a string`);
  return value;
}
function optionalInputString(context, name) {
  const value = context.input[name];
  if (value === undefined)
    return;
  if (typeof value !== "string")
...
function optionalInputNumber(context, name) {
  const value = context.input[name];
  if (value === undefined)
    return;
  if (typeof value !== "number" || !Number.isSafeInteger(value))
    throw new Error(`input.${name} must be a safe integer`);
  return value;
}
function optionalInputBoolean(context, name) {
  const value = context.input[name];
  if (value === undefined)
    return;
...
function officialMadeWithAiLabel(madeWithAi) {
  return madeWithAi === true ? true : undefined;
}
function inputStrings(context, name) {
  const value = context.input[name];
...
function optionalInputStrings(context, name) {
  if (context.input[name] === undefined)
    return;
  return inputStrings(context, name);
}
function inputNumbers(context, name) {
  const value = context.input[name];
...
function exactXId(value, label) {
  if (!isXAccountSubject(value))
    throw new Error(`${label} must be a 1-19 digit X object ID`);
  return value;
}
function inputXId(context, name) {
  return exactXId(inputString(context, name), `input.${name}`);
}
function optionalInputXId(context, name) {
  const value = optionalInputString(context, name);
  return value === undefined ? undefined : exactXId(value, `input.${name}`);
}
function exactDmConversationId(value, label) {
  ...
  if (value.includes("-")) {
    ...
    if (left === right)
      throw new Error(`${label} must identify two different legacy X DM participants`);
  }
  return value;
}
function exactChatConversationId(value, label) {
  ...
  if (value.includes("-")) {
    const [left, right] = value.split("-");
    if (left === right)
      throw new Error(`${label} must identify two different X Chat participants`);
  }
  return value;
}
function exactChatEventConversationId(value, label) {
  if (!/^(?:[0-9]{1,19}:[0-9]{1,19}|g[0-9]{1,19})$/u.test(value)) {
    throw new Error(`${label} must be an exact X Chat event conversation ID`);
  }
  return value;
}
function exactEncryptedChatEnvelope(value, label) {
  if (typeof value !== "string" || value.length === 0 || value.length > MAX_CHAT_ENVELOPE_CHARACTERS || value.length % 4 !== 0 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/u.test(value) || Buffer.from(value, "base64").toString("base64") !== value) {
    throw new Error(`official X ${label} contained an invalid encrypted envelope`);
  }
  return value;
}
function exactUniqueXIds(values, label) {
  const ids = values.map((value, index) => exactXId(value, `${label}[${index}]`));
  if (new Set(ids).size !== ids.length)
    throw new Error(`${label} must contain unique X object IDs`);
  return ids;
}
function bearer(context, json = false) {
...
function richChatConversationParams(url) {
...
async function request(context, url, init, statuses) {
  ...
  if (method !== "GET" && method !== "HEAD") {
    ...
    if (isRecord(body) && body.errors !== undefined) {
      if (!Array.isArray(body.errors))
        throw new Error("official X mutation response returned invalid errors");
      if (body.errors.length > 0)
        throw new Error("official X mutation response returned provider errors");
    }
  }
  return response;
}
async function authenticatedUserId(context) {
  ...
  const id = exactXId(requiredString(dataRecord(response, "authenticated-user"), "id", "authenticated-user"), "official X authenticated-user id");
...
async function authenticatedWriteUserId(context) {
...
async function authenticatedPrivateReadUserId(context) {
  if (context.auth.subject === undefined) {
...
function providerPage(response, requestedLimit, returned, providerReturned) {
  const body = recordBody(response, "page");
  if (body.meta !== undefined && !isRecord(body.meta))
    throw new Error("official X page returned invalid metadata");
  const meta = isRecord(body.meta) ? body.meta : null;
  for (const name of ["next_token", "previous_token"]) {
...
function exactDirectDmConversationSet(value, label) {
  const id = exactDmConversationId(value, label);
  if (!id.includes("-"))
    throw new Error(`${label} did not identify a direct legacy X DM conversation`);
  return new Set(id.split("-"));
}
function sameIdSet(left, right) {
  return left.size === right.size && [...left].every((id) => right.has(id));
}
function chatProviderPage(response, requestedLimit, returned) {
  ...
  const meta = metaPresent ? body.meta : {};
...
function exactItemIds(items, label) {
  ...
  if (new Set(ids).size !== ids.length)
    throw new Error(`official X ${label} response contained duplicate item IDs`);
  return ids;
}
function postUrl(id) {
  return `https://x.com/i/web/status/${encodedSegment(id)}`;
}
async function executeFeedsRead(context) {
  ...
  if (["home-reverse-chronological", "user", "mentions", "recent-search"].includes(feed)) {
    for (const [inputName, queryName] of [
      ...
      const value = inputName === "since_id" || inputName === "until_id" ? optionalInputXId(context, inputName) : optionalInputString(context, inputName);
      if (value !== undefined)
  ...
  if (feed === "home-reverse-chronological" || feed === "user") {
    const exclusions = [];
    if (optionalInputBoolean(context, "exclude_replies") === true)
      exclusions.push("replies");
    if (optionalInputBoolean(context, "exclude_reposts") === true)
...
async function executePostsRead(context) {
  ...
  if (ids.length === 1)
    context.setFinalUrl(postUrl(ids[0] ?? ""));
}
async function executeCommentsRead(context) {
  const postId = inputXId(context, "post_id");
  const requestedLimit = optionalInputNumber(context, "limit") ?? 10;
  if (requestedLimit < 10)
    throw new Error("input.limit must be at least 10 for official X recent reply search");
  const window = optionalInputString(context, "window") ?? "recent-7-days";
  const url = new URL(window === "recent-7-days" ? `${X_API_ORIGIN}/2/tweets/search/recent` : window === "full-archive" ? `${X_API_ORIGIN}/2/tweets/search/all` : (() => {
  ...
  if (allItems.length > requestedLimit)
    throw new Error("official X reply search exceeded the requested result bound");
  const items = allItems;
  ...
  context.setOutput({
    ...
    coverage: {
      complete: false,
      limitation: window === "recent-7-days" ? "Replies are reconstructed with recent search, which covers at most seven days; this is never a complete conversation archive." : "Replies are reconstructed with entitled full-archive search and opaque pagination; search results are never labeled a provably complete conversation archive.",
      window
    }
  });
...
async function executeMessagingList(context) {
  ...
  if (view === "all") {
    ...
    if (participantId === context.auth.subject)
  ...
  if (view === "chat-conversations")
    richChatConversationParams(url);
  else if (view === "chat-events")
    url.searchParams.set("chat_message_event.fields", CHAT_MESSAGE_EVENT_FIELDS);
  else
    richDmParams(url);
  ...
  if (items.length > requestedLimit)
    throw new Error(`official X ${label} response exceeded the requested result bound`);
  if (view === "chat-conversations") {
    const ids = items.map((value, index) => {
    ...
    if (new Set(ids).size !== ids.length)
      throw new Error("official X Chat conversation response contained duplicate IDs");
  } else if (view === "chat-events") {
    const ids = items.map((value, index) => {
    ...
    if (new Set(ids).size !== ids.length)
      throw new Error("official X Chat event response contained duplicate IDs");
  } else {
    ...
    const requestedConversation = view === "conversation" ? exactDmConversationId(inputString(context, "target_id"), "input.target_id") : null;
    for (const [index, value] of items.entries()) {
      const item = record(value, `DM-event item ${index + 1}`);
      const conversationId = exactDmConversationId(requiredString(item, "dm_conversation_id", "DM-event item"), "official X DM-event conversation id");
      ...
      if (requestedParticipant !== null) {
        const returnedParticipants = exactDirectDmConversationSet(conversationId, "official X DM-event conversation id");
  ...
  if (view === "chat-events") {
    ...
    for (const [index, value] of items.entries()) {
      ...
      if (item.sender_id !== undefined)
        exactXId(requiredString(item, "sender_id", "Chat event item"), "official X Chat sender id");
      ...
      if (item.created_at_msec !== undefined && (typeof item.created_at_msec !== "string" || !/^[0-9]{1,20}$/u.test(item.created_at_msec))) {
        throw new Error("official X Chat event response contained an invalid creation timestamp");
      }
      const returned = exactChatEventConversationId(requiredString(item, "conversation_id", "Chat event item"), "official X Chat event conversation id");
      const matches = target.startsWith("g") ? returned === target : (() => {
        if (returned.startsWith("g"))
          return false;
        ...
        if (targetUsers.size !== 2)
          throw new Error("input.target_id must identify another X Chat participant");
        return returnedUsers.size === 2 && [...targetUsers].every((id) => returnedUsers.has(id)) && [...returnedUsers].every((id) => targetUsers.has(id));
      })();
      if (!matches)
        throw new Error("official X Chat event response contained an event outside the requested conversation");
    }
  ...
  if (view === "chat-conversations" && meta.has_message_requests !== undefined && typeof meta.has_message_requests !== "boolean") {
  ...
  context.setOutput({
    ...
    includes: providerIncludes(response),
    providerErrors: providerErrors(response),
    page: chatView ? chatProviderPage(response, requestedLimit, items.length) : providerPage(response, requestedLimit, items.length, items.length),
    ...chatView ? { meta } : {},
    coverage: chatView ? {
      complete: false,
      encryptedChat: true,
      plaintextAvailable: false,
      ...view === "chat-conversations" && typeof meta.has_message_requests === "boolean" ? { messageRequestsPending: meta.has_message_requests } : {},
      limitation: view === "chat-conversations" ? "The official Chat index exposes conversation metadata and whether requests are pending, but not separate Requests/Priority/Hidden folder collections or plaintext messages." : "Official Chat events contain signed encrypted envelopes; ghostget returns the provider envelope without claiming to decrypt or verify its plaintext."
    } : {
      complete: false,
...
async function executeMessagingRead(context) {
  ...
  if (returnedId !== eventId)
    throw new Error("official X DM-event lookup returned an unexpected event ID");
  context.setOutput({
    provider: "x",
...
function validateMediaFile(file) {
  ...
  if (!/^[a-f0-9]{64}$/u.test(file.sha256))
    throw new Error("official X attachment has an invalid confirmed digest");
}
...
function sameProviderFile(left, right) {
...
function openProviderFile(file, expectedIdentity) {
  validateMediaFile(file);
  if (!isAbsolute(file.path))
    throw new Error("official X attachment resolver returned a non-absolute path");
  let descriptor;
  try {
    descriptor = openSync(file.path, constants.O_RDONLY | ("O_NOFOLLOW" in constants ? constants.O_NOFOLLOW : 0) | ("O_NONBLOCK" in constants ? constants.O_NONBLOCK : 0));
  } catch {
...
function closeProviderFile(descriptor) {
  try {
    closeSync(descriptor);
  } catch {
...
function verifiedProviderRead(file, expectedIdentity, materialize) {
  ...
  try {
    while (offset < file.bytes) {
      ...
      if (destination === null)
        throw new Error("official X attachment verification buffer is unavailable");
      const destinationOffset = output === null ? 0 : offset;
      const length = Math.min(output === null ? destination.byteLength : FILE_VERIFY_CHUNK_BYTES, file.bytes - offset);
      const count = readSync(opened.descriptor, destination, destinationOffset, length, offset);
      if (count < 1)
        throw new Error("official X attachment ended before its confirmed byte length");
...
function preflightProviderFile(file) {
  const verified = verifiedProviderRead(file, undefined, false);
  return { file, preflightIdentity: verified.identity };
}
function preflightProviderFiles(files) {
  return files.map(preflightProviderFile);
}
function mediaCategory(mediaType, destination) {
  if (mediaType === "image/jpeg" || mediaType === "image/png")
    return `${destination}_image`;
  if (mediaType === "image/gif")
    return `${destination}_gif`;
  if (mediaType === "video/mp4")
    return `${destination}_video`;
  throw new Error(`official X media upload does not support ${mediaType}`);
}
function genericUploadName(mediaType) {
...
function multipartBearer(context, contentType) {
  const headers = bearer(context);
  headers.set("Content-Type", contentType);
  return headers;
}
function mediaId(response, label) {
  return exactXId(requiredString(dataRecord(response, label), "id", label), `official X ${label} id`);
}
async function uploadOneShotImage(context, prepared, category) {
  const { file } = prepared;
  const bytes = verifiedProviderRead(file, prepared.preflightIdentity, true).bytes;
  if (bytes === null)
    throw new Error("official X image could not be materialized safely");
  ...
  const response = await request(context, `${X_API_ORIGIN}/2/media/upload`, {
    method: "POST",
...
async function waitForMedia(context, id, initial) {
  let current = processingState(initial);
  if (current.state === null || current.state === "succeeded")
    return;
  if (current.state === "failed")
    throw new Error("official X media processing failed");
...
async function uploadChunkedMedia(context, prepared, category) {
  const { file } = prepared;
  if (file.mediaType === "image/gif") {
    const bytes = verifiedProviderRead(file, prepared.preflightIdentity, true).bytes;
    if (bytes === null)
      throw new Error("official X GIF could not be materialized safely");
    rejectGifProvenanceMarkers(bytes);
  }
  const opened = openProviderFile(file, prepared.preflightIdentity);
  let id;
  try {
    const initialize = await request(context, `${X_API_ORIGIN}/2/media/upload/initialize`, {
      method: "POST",
    ...
    while (offset < file.bytes) {
      ...
      while (readOffset < length) {
        ...
        if (count < 1)
          throw new Error("official X attachment ended during chunked upload");
        readOffset += count;
      }
      hash.update(buffer);
      const multipart = xMultipartPayload([{ name: "segment_index", value: String(segmentIndex) }], {
        bytes: buffer,
        mediaType: file.mediaType,
        filename: genericUploadName(file.mediaType)
      });
      await request(context, `${X_API_ORIGIN}/2/media/upload/${encodedSegment(id)}/append`, {
        method: "POST",
        headers: multipartBearer(context, multipart.contentType),
        body: new Uint8Array(multipart.body)
      }, [200]);
  ...
  const finalized = await request(context, `${X_API_ORIGIN}/2/media/upload/${encodedSegment(id)}/finalize`, { method: "POST", headers: bearer(context) }, [200]);
  await waitForMedia(context, id, finalized);
  return { id, category, mediaType: file.mediaType, bytes: file.bytes };
}
async function uploadMedia(context, prepared, destination) {
  const { file } = prepared;
  validateMediaFile(file);
  const category = mediaCategory(file.mediaType, destination);
  return file.mediaType === "image/jpeg" || file.mediaType === "image/png" ? uploadOneShotImage(context, prepared, category) : uploadChunkedMedia(context, prepared, category);
}
async function applyMediaAltText(context, media, altText) {
  if (altText === undefined)
    return media;
  const response = await request(context, `${X_API_ORIGIN}/2/media/metadata`, {
    method: "POST",
    headers: bearer(context, true),
  ...
  if (returnedId !== media.id)
    throw new Error("official X media metadata response returned an unexpected media ID");
  const associatedMetadata = record(result.associated_metadata, "media metadata associated_metadata");
  const returnedAltText = requiredString(record(associatedMetadata.alt_text, "media metadata alt_text"), "text", "media metadata alt_text");
  if (returnedAltText !== altText) {
    throw new Error("official X media metadata response returned unexpected alternative text");
  }
  return { ...media, altText };
}
function alignedAltTexts(context, inputName, mediaCount) {
  const values = optionalInputStrings(context, inputName);
  if (values === undefined)
    return Array.from({ length: mediaCount }, () => {
      return;
    });
  if (values.length !== mediaCount)
    throw new Error(`input.${inputName} must align one-to-one with the reviewed media`);
  return values;
}
function validateImageAltTexts(files, altTexts, inputName) {
...
async function executePostsPublish(context) {
  ...
  if (Array.isArray(pollOptions) && files.length > 0)
    throw new Error("input.media and poll inputs are mutually exclusive");
  ...
  await context.dispatch(async () => {
    ...
    if (uploaded.length > 0)
      payload.media = { media_ids: uploaded.map((media) => media.id) };
    if (Array.isArray(pollOptions) && pollDuration !== undefined) {
    ...
    const response = await request(context, `${X_API_ORIGIN}/2/tweets`, { method: "POST", headers: bearer(context, true), body: JSON.stringify(payload) }, [201]);
...
async function executeReplyCreate(context) {
  ...
  await context.dispatch(async () => {
    ...
    const payload = { reply: { in_reply_to_tweet_id: targetPostId } };
    if (body !== undefined)
      payload.text = body;
    if (uploaded.length > 0)
      payload.media = { media_ids: uploaded.map((media) => media.id) };
...
async function executeThreadPublish(context) {
  const items = inputStrings(context, "items");
  const files = await context.resolveFiles("media");
  const mediaIndices = files.length === 0 && context.input.media_item_indices === undefined ? [] : inputNumbers(context, "media_item_indices");
  ...
  for (const group of groupedFiles.values())
    validatePostMedia(group);
  const grouped = new Map;
  const preparedFiles = preflightProviderFiles(files);
  for (const [index, prepared] of preparedFiles.entries()) {
    const itemIndex = mediaIndices[index];
    if (itemIndex === undefined)
      throw new Error("reviewed thread media lost its validated item binding");
  ...
  if (first !== undefined)
    context.setFinalUrl(first.url);
}
async function executeMessagingSend(context) {
  ...
  const targetId = targetKind === "participant" ? inputXId(context, "target_id") : targetKind === "conversation" ? exactDmConversationId(inputString(context, "target_id"), "input.target_id") : null;
  if (targetId === null)
    throw new Error("input.target_kind names an unsupported official X DM target");
  if (targetKind === "participant" && targetId === context.auth.subject) {
    throw new Error("input.target_id must identify another X DM participant");
  }
  const path = targetKind === "participant" ? `/2/dm_conversations/with/${encodedSegment(targetId)}/messages` : `/2/dm_conversations/${encodedSegment(targetId)}/messages`;
  const files = await context.resolveFiles("media");
  if (files.length > 1)
    throw new Error("official X Direct Messages support at most one attachment");
  const file = files[0];
  const mediaAltText = optionalInputString(context, "media_alt_text");
  if (mediaAltText !== undefined && file === undefined)
    throw new Error("input.media_alt_text requires input.media");
  if (mediaAltText !== undefined && file !== undefined && file.mediaType !== "image/jpeg" && file.mediaType !== "image/png") {
  ...
  await context.dispatch(async () => {
    ...
    if (body !== undefined)
      payload.text = body;
    if (uploaded !== null)
      payload.attachments = [{ media_id: uploaded.id }];
    const response = await request(context, `${X_API_ORIGIN}${path}`, { method: "POST", headers: bearer(context, true), body: JSON.stringify(payload) }, [201]);
    const result = dataRecord(response, "DM send");
    const conversationId = exactDmConversationId(requiredString(result, "dm_conversation_id", "DM send"), "official X sent DM conversation id");
...
async function executeRepost(context) {
  const postId = inputXId(context, "post_id");
  const me = await authenticatedWriteUserId(context);
  const enabled = context.input.enabled === true;
  await context.dispatch(async () => {
    const response = enabled ? await request(context, `${X_API_ORIGIN}/2/users/${encodedSegment(me)}/retweets`, { method: "POST", headers: bearer(context, true), body: JSON.stringify({ tweet_id: postId }) }, [200]) : await request(context, `${X_API_ORIGIN}/2/users/${encodedSegment(me)}/retweets/${encodedSegment(postId)}`, { method: "DELETE", headers: bearer(context) }, [200]);
...
async function executeBookmark(context) {
  const postId = inputXId(context, "post_id");
  const me = await authenticatedWriteUserId(context);
  const enabled = context.input.enabled === true;
  await context.dispatch(async () => {
    const response = enabled ? await request(context, `${X_API_ORIGIN}/2/users/${encodedSegment(me)}/bookmarks`, { method: "POST", headers: bearer(context, true), body: JSON.stringify({ tweet_id: postId }) }, [200]) : await request(context, `${X_API_ORIGIN}/2/users/${encodedSegment(me)}/bookmarks/${encodedSegment(postId)}`, { method: "DELETE", headers: bearer(context) }, [200]);
...
async function executeArticle(context, operation) {
  const retainedV2DraftOnly = operation === "articles.publish" && context.recipe.contractVersion === 2 && context.input.draft_only === true;
  ...
  if (covers.length > 1)
    throw new Error("official X Articles support one cover image");
  const cover = covers[0];
  const coverAltText = optionalInputString(context, "cover_alt_text");
  if (coverAltText !== undefined && cover === undefined)
    throw new Error("input.cover_alt_text requires input.cover");
  let preparedCover;
  if (cover !== undefined) {
  ...
  await context.dispatch(async () => {
    ...
    const draftResponse = await request(context, `${X_API_ORIGIN}/2/articles/draft`, { method: "POST", headers: bearer(context, true), body: JSON.stringify(draftPayload) }, [201]);
    ...
    context.setOutput({
      ...
      operation,
      published: false,
      mode: operation === "articles.draft.save" || retainedV2DraftOnly ? "draft" : "publish",
      draftId: articleId,
      draft,
      uploadedCover: uploaded
    });
    if (operation === "articles.draft.save" || retainedV2DraftOnly)
      return;
    const publishResponse = await request(context, `${X_API_ORIGIN}/2/articles/${encodedSegment(articleId)}/publish`, { method: "POST", headers: bearer(context) }, [200]);
...
async function executeXProvider(context) {
  ...
  if (action === "feeds.read")
    await executeFeedsRead(context);
  else if (action === "posts.read")
    await executePostsRead(context);
  else if (action === "comments.read")
    await executeCommentsRead(context);
  ...
  else if (action === "messaging.send")
    await executeMessagingSend(context);
  else if (action === "posts.publish")
    await executePostsPublish(context);
  else if (action === "replies.create")
    await executeReplyCreate(context);
  else if (action === "threads.publish")
    await executeThreadPublish(context);
  else if (action === "posts.repost")
    await executeRepost(context);
  else if (action === "content.save")
    await executeBookmark(context);
  else if (action === "articles.draft.save")
    await executeArticle(context, action);
  else if (action === "articles.publish")
    await executeArticle(context, action);
  else
    throw new Error(`official X provider action ${action} is not implemented`);
}
export {
  executeXProvider
};
