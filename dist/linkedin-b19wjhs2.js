...
var LINKEDIN_API_ORIGIN = "https://api.linkedin.com";
var LINKEDIN_API_HOSTS = ["api.linkedin.com"];
var LINKEDIN_VERSION = "202606";
var LINKEDIN_PROTOCOL_VERSION = "2.0.0";
var MAX_IMAGE_BYTES = 5 * 1024 * 1024;
var MAX_DOCUMENT_BYTES = 100 * 1024 * 1024;
var MAX_VIDEO_BYTES = 500 * 1024 * 1024;
var MIN_VIDEO_BYTES = 75000;
var FILE_READ_CHUNK_BYTES = 1024 * 1024;
var MAX_MEDIA_STATUS_POLLS = 100;
var MAX_MEDIA_STATUS_WAIT_MS = 8 * 60000;
var DEFAULT_MEDIA_STATUS_DELAY_MS = 1000;
var CONNECTIONS_DEFAULT_COUNT = 10;
var CONNECTIONS_MAX_COUNT = 50;
var CONNECTIONS_MAX_START = 1e5;
var CONNECTIONS_PROJECTION = "(elements(*(to~)),paging)";
var CONNECTION_STATS_UNAVAILABLE_REASON = "linkedin-connections-message-statistics-unavailable";
var imageMediaTypes = new Set(["image/jpeg", "image/png", "image/gif"]);
var videoMediaTypes = new Set(["video/mp4"]);
var documentMediaTypes = new Set([
  "application/pdf",
...
var unavailableConnectionStats = Object.freeze({
  count: null,
  complete: false,
  lowerBound: false,
  truncated: false,
...
var unavailableConnectionStatsProjection = projectContactDirectionStats(unavailableConnectionStats, unavailableConnectionStats);
...
function hasAnyAsciiControl(value) {
  for (let index = 0;index < value.length; index += 1) {
    const code = value.charCodeAt(index);
    if (code <= 31 || code === 127)
      return true;
...
function requiredString(value, label, maximum = 8192, allowEmpty = false) {
  if (typeof value !== "string" || !allowEmpty && value.length === 0 || value.length > maximum || hasUnsafeTextControl(value))
    throw new Error(`${label} must be a bounded string without control characters`);
  return value;
}
function optionalString(value, label, maximum = 8192) {
...
function responseString(value, label, maximum = 8192, allowEmpty = false) {
...
function hasUnsafeStructuredTextControl(value) {
  for (let index = 0;index < value.length; index += 1) {
    const code = value.charCodeAt(index);
...
function structuredResponseText(value, label, maximum) {
  if (typeof value !== "string" || value.length === 0 || Buffer.byteLength(value, "utf8") > maximum || value.trim().length === 0 || !isWellFormedText(value) || hasUnsafeStructuredTextControl(value))
    throw new Error(`LinkedIn returned an invalid ${label}`);
  return value;
}
function personUrn(value, label) {
...
function localizedConnectionName(value, label) {
  ...
  if (source.preferredLocale !== undefined) {
    ...
    const language = structuredResponseText(locale.language, `${label}.preferredLocale.language`, 2);
    const country = locale.country === undefined ? null : structuredResponseText(locale.country, `${label}.preferredLocale.country`, 2);
  ...
  for (const key of entries) {
    ...
    localized[key] = structuredResponseText(localizedSource[key], `${label}.localized.${key}`, 512);
  }
  let selectedLocale = null;
  let selectionBasis = "unavailable";
  if (preferredLocale !== null) {
    const preferredKey = preferredLocale.country === null ? preferredLocale.language : `${preferredLocale.language}_${preferredLocale.country}`;
    if (Object.hasOwn(localized, preferredKey)) {
      selectedLocale = preferredKey;
      selectionBasis = "preferred-locale-exact";
    } else {
      const compatibleVariants = entries.filter((key) => key.startsWith(`${preferredKey}_`));
...
function exactContactsInput(input) {
  ...
  return Object.freeze({
    ...
    count: inputInteger(source.count, "input.count", CONNECTIONS_DEFAULT_COUNT, 1, CONNECTIONS_MAX_COUNT)
  });
}
function contactsAccountSubject(context) {
  const subject = context.auth.subject;
  if (typeof subject !== "string" || !/^urn:li:person:[A-Za-z0-9_-]{1,256}$/u.test(subject)) {
    throw new Error("LinkedIn contacts.list requires an OAuth locator with an exact person URN subject");
  }
  context.addRequiredScopes(["r_1st_connections", "r_liteprofile"]);
  return subject;
}
function restApiHeaders(context, json = false) {
...
function socialTargetUrn(value) {
  if (/^urn:li:(?:activity|share|ugcPost):[0-9]{1,32}$/u.test(value) || /^urn:li:comment:\(urn:li:activity:[0-9]{1,32},[0-9]{1,32}\)$/u.test(value))
    return value;
  throw new Error("LinkedIn social target must be an exact post, activity, or composite comment URN");
}
function commentReadTargetUrn(value) {
  if (/^urn:li:(?:share|ugcPost):[0-9]{1,32}$/u.test(value) || /^urn:li:comment:\(urn:li:activity:[0-9]{1,32},[0-9]{1,32}\)$/u.test(value))
    return value;
  throw new Error("LinkedIn comment target must be an exact share, UGC Post, or composite comment URN");
}
function requireWriteActor(context, actor, scopes) {
...
function linkedInPostUrl(id) {
  return `https://www.linkedin.com/feed/update/${postUrn(id)}/`;
}
function linkedInCommentUrl(target, createdCommentUrn) {
  const url = new URL(linkedInPostUrl(target));
  if (createdCommentUrn !== null)
    url.searchParams.set("commentUrn", createdCommentUrn);
  return url.toString();
}
function scalarResponseString(record_, key) {
  const value = record_[key];
  return typeof value === "string" && value.length <= 8192 ? value : null;
}
function scalarResponseNumber(record_, key) {
  const value = record_[key];
  return typeof value === "number" && Number.isFinite(value) ? value : null;
}
function optionalResponseString(record_, key, label, maximum = 8192) {
  return record_[key] === undefined ? null : responseString(record_[key], label, maximum);
}
function exactResponsePost(value, expectedId) {
  const post = record(value, "post resource");
  const id = postUrn(responseString(post.id, "post ID", 500));
  if (id !== expectedId)
    throw new Error("LinkedIn returned a post other than the requested post URN");
  return normalizePost(post);
}
function exactAuthorPost(value, expectedAuthor) {
...
function normalizeComment(value) {
  ...
  const lastModified = comment.lastModified === undefined ? null : record(comment.lastModified, "comment modification stamp");
  return {
...
function exactCommentIdentity(comment, label) {
  const id = responseString(comment.id, `${label} ID`, 32);
  if (!/^[0-9]{1,32}$/u.test(id))
    throw new Error(`LinkedIn returned an invalid ${label} ID`);
  const urn = commentUrn(responseString(comment.commentUrn, `${label} URN`, 1000));
  if (!urn.endsWith(`,${id})`)) {
    throw new Error(`LinkedIn returned inconsistent ${label} ID and URN fields`);
  }
  return { id, urn };
}
function exactListedComment(value, target, nested) {
  ...
  rootObjectUrn(responseString(comment.object, `${nested ? "reply" : "comment"} object`, 1000));
  const parent = optionalResponseString(comment, "parentComment", "parent comment URN", 1000);
  if (nested) {
...
function exactConnection(value, index) {
  ...
  const firstName = localizedConnectionName(decorated.firstName, `${label}.to~.firstName`);
  const lastName = localizedConnectionName(decorated.lastName, `${label}.to~.lastName`);
  return Object.freeze({
    providerId: urn,
    personId: id,
    displayName: firstName.value === null || lastName.value === null ? null : `${firstName.value} ${lastName.value}`,
...
function exactQueryEntries(url) {
  return [...url.searchParams.entries()].filter(([key]) => key !== "start" && key !== "count").map(([key, value]) => `${key}\x00${value}`).sort();
}
function exactPagingParameter(url, key) {
...
function validatedNextLink(linkValue, requestUrl) {
  const link = record(linkValue, "next paging link");
  const href = responseString(link.href, "next paging link href", 8192);
  const next = new URL(href, LINKEDIN_API_ORIGIN);
  if (next.protocol !== "https:" || next.hostname !== "api.linkedin.com" || next.port !== "" || next.username !== "" || next.password !== "" || next.hash !== "" || next.pathname !== requestUrl.pathname || JSON.stringify(exactQueryEntries(next)) !== JSON.stringify(exactQueryEntries(requestUrl)))
    throw new Error("LinkedIn returned a next paging link for a different collection");
  return next;
}
function connectionPageOutput(responseBody, accountSubject, requestedStart, requestedCount, requestUrl) {
  ...
  const elements = strictArray(body.elements, "LinkedIn connections response.elements", requestedCount);
  const paging = strictRecord(body.paging, "LinkedIn connections response.paging");
  exactKeys(paging, ["start", "count", "total", "links"], [], "LinkedIn connections response.paging");
  ...
  if (elements.length !== expectedReturned) {
    throw new Error("LinkedIn connections page length contradicted its exact paging total");
  }
  const links = strictArray(paging.links, "LinkedIn connections response.paging.links", 2);
  ...
  for (let index = 0;index < links.length; index += 1) {
    ...
    const href = structuredResponseText(link.href, `${label}.href`, 8192);
    if (/\s/u.test(href))
      throw new Error("LinkedIn returned an invalid connections paging URL");
    ...
    const expectedStart = relation === "next" ? start + count : Math.max(0, start - count);
    if (linkedStart !== expectedStart || relation === "next" && start + elements.length >= total || relation === "prev" && start === 0) {
      throw new Error(`LinkedIn returned a contradictory ${relation} connections paging link`);
    }
  }
  const contacts = Object.freeze(elements.map((value, index) => exactConnection(value, index)));
  ...
  const nextStart = candidateNextStart !== null && candidateNextStart <= CONNECTIONS_MAX_START ? candidateNextStart : null;
  const offsetBoundReached = candidateNextStart !== null && nextStart === null;
  const contactSetComplete = start === 0 && contacts.length === total;
  const contactSetCompleteness = contactSetComplete ? "complete" : offsetBoundReached ? "bounded" : nextStart === null ? "terminal-page" : "page";
  return Object.freeze({
    schemaVersion: 1,
    provider: "linkedin",
...
function nextOffset(pagingValue, requestedStart, requestedCount, returnedCount, requestUrl) {
  ...
  if (start !== requestedStart)
    throw new Error("LinkedIn paging start did not match the requested page");
  ...
  for (const linkValue of linksValue ?? []) {
    ...
    if (rel !== "next")
      continue;
    if (nextLink !== null)
      throw new Error("LinkedIn returned multiple next paging links");
  ...
  if (!Number.isSafeInteger(offset) || offset <= requestedStart || offset > 1e5) {
...
function pageOutput(operation, coverage, responseBody, start, count, requestUrl, normalize, limitations) {
  ...
  const identities = items.map((item) => item.id).filter((id) => typeof id === "string");
...
function singleOutput(operation, item) {
  return {
    schemaVersion: 1,
    provider: "linkedin",
    operation,
...
function writeOutput(operation, result, limitations = []) {
  return {
    schemaVersion: 1,
    provider: "linkedin",
    operation,
...
async function resharePost(context) {
  ...
  const commentary = requiredString(context.input.body, "input.body", 3000);
  const visibility = context.input.visibility === undefined ? "PUBLIC" : requiredString(context.input.visibility, "input.visibility", 20);
...
function openProviderFile(file, maximumBytes, expectedIdentity, minimumBytes = 1) {
  if (!isAbsolute(file.path))
    throw new Error("LinkedIn media resolver returned a non-absolute path");
  ...
  const descriptor = openSync(file.path, constants.O_RDONLY | ("O_NOFOLLOW" in constants ? constants.O_NOFOLLOW : 0) | ("O_NONBLOCK" in constants ? constants.O_NONBLOCK : 0));
  try {
    const stats = fstatSync(descriptor, { bigint: true });
    const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
    if (!stats.isFile() || stats.size !== BigInt(file.bytes) || uid !== undefined && stats.uid !== BigInt(uid) || process.platform !== "win32" && (stats.mode & 0o777n) !== 0o600n)
...
function readVerifiedFile(file, maximumBytes, materialize, minimumBytes = 1) {
  ...
  try {
    while (offset < file.bytes) {
      ...
      if (destination === null)
        throw new Error("LinkedIn media read buffer is unavailable");
      const destinationOffset = output === null ? 0 : offset;
      const length = Math.min(output === null ? destination.byteLength : FILE_READ_CHUNK_BYTES, file.bytes - offset);
      const count = readSync(opened.descriptor, destination, destinationOffset, length, offset);
      if (count < 1)
        throw new Error("LinkedIn media ended before its reviewed byte length");
    ...
    if (!sameFile(opened.stats, after))
      throw new Error("LinkedIn media changed while it was validated");
    if (hash.digest("hex") !== file.sha256)
      throw new Error("LinkedIn media no longer matches the confirmed attachment digest");
    return { identity: opened.stats, bytes: output };
  } finally {
    closeSync(opened.descriptor);
  }
}
async function prepareMedia(context) {
  ...
  if (files.length !== rawMedia.length)
    throw new Error("LinkedIn media did not resolve every reviewed file");
  const allImages = files.every((file2) => imageMediaTypes.has(file2.mediaType));
  if (allImages) {
    const items = files.map((file2) => {
      const bytes = readVerifiedFile(file2, MAX_IMAGE_BYTES, true).bytes;
      if (bytes === null)
        throw new Error("LinkedIn image could not be materialized");
      return { file: file2, bytes };
    });
  ...
  const file = files[0];
  if (file === undefined)
    throw new Error("LinkedIn media resolver omitted its file");
  if (documentMediaTypes.has(file.mediaType)) {
    ...
    if (bytes === null)
      throw new Error("LinkedIn document could not be materialized");
...
function validatePostAuxiliaryInputs(context, media) {
  ...
  const articleDescription = optionalString(context.input.article_description, "input.article_description", 4000);
  const altText = optionalString(context.input.alt_text, "input.alt_text", 4000);
  if (articleUrl !== null) {
    if (media.kind !== "none")
      throw new Error("LinkedIn article cards and uploaded media are mutually exclusive");
    if (articleTitle === null)
      throw new Error("input.article_title is required for a LinkedIn external article card");
  ...
  return {
    article: articleUrl === null || articleTitle === null ? null : {
      source: articleUrl,
      title: articleTitle,
...
function uploadUrl(value) {
  ...
  if (url.protocol !== "https:" || url.username !== "" || url.password !== "" || url.port !== "" || !allowedHost || !allowedPath)
    throw new Error("LinkedIn returned an unapproved media upload destination");
  return url;
}
async function putUploadBytes(context, destination, bytes, expectedStatuses, headers) {
...
async function uploadAuthenticatedBytes(context, destination, bytes, expectedStatuses) {
  return putUploadBytes(context, destination, bytes, expectedStatuses, bearerHeaders(context.token.accessToken, { "Content-Type": "application/octet-stream" }));
}
async function uploadSignedVideoPart(context, destination, bytes) {
  return putUploadBytes(context, destination, bytes, [200, 201], new Headers({ "Content-Type": "application/octet-stream" }));
}
function mediaStatusDelay(headers) {
...
async function waitForMediaAvailable(context, kind, id) {
  const mediaDeadline = Date.now() + MAX_MEDIA_STATUS_WAIT_MS;
  for (let attempt = 0;attempt < MAX_MEDIA_STATUS_POLLS; attempt += 1) {
    const response = await context.http.request(apiUrl(`/rest/${kind}/${encodedUrn(id)}`), { method: "GET", headers: restApiHeaders(context) }, [200], LINKEDIN_API_HOSTS);
    ...
    if (attempt + 1 >= MAX_MEDIA_STATUS_POLLS || remaining <= 0)
      break;
...
async function initializeAndUploadImage(context, owner, bytes) {
  const response = await context.http.request(apiUrl("/rest/images?action=initializeUpload"), {
    method: "POST",
    headers: restApiHeaders(context, true),
...
async function initializeAndUploadDocument(context, owner, bytes) {
  const response = await context.http.request(apiUrl("/rest/documents?action=initializeUpload"), {
    method: "POST",
    headers: restApiHeaders(context, true),
    body: jsonBody({ initializeUploadRequest: { owner } })
  }, [200], LINKEDIN_API_HOSTS);
...
function uploadInstructions(value, fileBytes) {
...
function safeEtag(value) {
  if (value === null || value.length < 1 || value.length > 8192 || value.trim() !== value || hasAnyAsciiControl(value) || /^W\//iu.test(value)) {
...
function readFileRange(descriptor, firstByte, lastByte) {
  ...
  while (offset < length) {
    ...
    if (count < 1)
      throw new Error("LinkedIn video ended before an authoritative upload range");
    offset += count;
  }
  return buffer;
}
async function initializeUploadAndFinalizeVideo(context, owner, prepared) {
  const { file } = prepared;
  const opened = openProviderFile(file, MAX_VIDEO_BYTES, prepared.preflightIdentity, MIN_VIDEO_BYTES);
  const hash = createHash("sha256");
  const etags = [];
  try {
    const initialized = await context.http.request(apiUrl("/rest/videos?action=initializeUpload"), {
      method: "POST",
      headers: restApiHeaders(context, true),
      body: jsonBody({
    ...
    if (!/^urn:li:video:[A-Za-z0-9_-]{1,256}$/u.test(id))
      throw new Error("LinkedIn returned an invalid video URN");
    const token = responseString(value.uploadToken, "video upload token", 4096, true);
    ...
    const after = fstatSync(opened.descriptor, { bigint: true });
    if (!sameFile(opened.stats, after))
      throw new Error("LinkedIn video changed during upload");
    if (hash.digest("hex") !== file.sha256) {
      throw new Error("LinkedIn video no longer matches the confirmed attachment digest");
    }
    await context.http.request(apiUrl("/rest/videos?action=finalizeUpload"), {
      method: "POST",
      headers: restApiHeaders(context, true),
      body: jsonBody({
...
function exactViewerPersonSubject(value) {
...
async function listConnections(context) {
  ...
  if (!Buffer.from(accountSubject, "utf8").equals(Buffer.from(locatorSubject, "utf8"))) {
    throw new Error("authenticated LinkedIn person does not match the OAuth locator subject");
  }
  ...
  context.setOutput(connectionPageOutput(response.body, accountSubject, input.start, input.count, url));
}
async function readPosts(context) {
  const mode = requiredString(context.input.mode, "input.mode", 20);
  const view = context.input.view === undefined ? "READER" : requiredString(context.input.view, "input.view", 20);
  if (view !== "READER" && view !== "AUTHOR")
    throw new Error("input.view must be READER or AUTHOR");
  if (mode === "one") {
    const id = postUrn(requiredString(context.input.post_urn, "input.post_urn", 500));
    const url2 = apiUrl(`/rest/posts/${encodedUrn(id)}`);
    url2.searchParams.set("viewContext", view);
    const response2 = await context.http.request(url2, {
      method: "GET",
      headers: restApiHeaders(context)
    }, [200], LINKEDIN_API_HOSTS);
    context.setOutput(singleOutput("posts.read", exactResponsePost(response2.body, id)));
    context.setFinalUrl(linkedInPostUrl(id));
    return;
  }
  if (mode !== "author")
    throw new Error("input.mode must be one or author");
  const author = requiredString(context.input.author, "input.author", 500);
  const kind = actorKind(author);
  context.addRequiredScopes([kind === "member" ? "r_member_social" : "r_organization_social"]);
  const start = inputInteger(context.input.start, "input.start", 0, 0, 1e5);
  const count = inputInteger(context.input.count, "input.count", 10, 1, 100);
  const sort = context.input.sort === undefined ? "LAST_MODIFIED" : requiredString(context.input.sort, "input.sort", 20);
  ...
  context.setOutput(pageOutput("posts.read", "author-posts", response.body, start, count, url, (value) => exactAuthorPost(value, author), [
...
async function readComments(context) {
  const target = commentReadTargetUrn(requiredString(context.input.target_urn, "input.target_urn", 1000));
  const start = inputInteger(context.input.start, "input.start", 0, 0, 1e5);
  const count = inputInteger(context.input.count, "input.count", 10, 1, 100);
  ...
  context.setOutput(pageOutput("comments.read", "comments", response.body, start, count, url, (value) => exactListedComment(value, target, target.startsWith("urn:li:comment:")), [
    ...
    ...!target.startsWith("urn:li:comment:") ? ["For top-level collections, requestedTarget records the exact share or UGC Post request path; LinkedIn's canonical response object/activity may legitimately differ and is preserved separately."] : []
  ]));
}
function createdCommentIdentity(bodyValue, headers, expected) {
  ...
  const urn = commentUrn(responseString(body.commentUrn, "created comment URN", 1000));
  ...
  if (actor !== expected.actor)
    throw new Error("LinkedIn created comment actor disagreed with the requested actor");
  const object = rootObjectUrn(responseString(body.object, "created comment object", 1000));
  if (object !== expected.object)
    throw new Error("LinkedIn created comment object disagreed with the requested object");
  const parent = optionalResponseString(body, "parentComment", "created comment parent", 1000);
  if (expected.parent === null) {
    if (parent !== null)
      throw new Error("LinkedIn created a nested reply instead of the requested top-level comment");
...
async function createComment(context, reply) {
  ...
  const object = rootObjectUrn(requiredString(context.input.object_urn, "input.object_urn", 1000));
  const body = requiredString(context.input.body, "input.body", 500);
  const target = reply ? commentUrn(requiredString(context.input.parent_comment_urn, "input.parent_comment_urn", 1000)) : postUrn(requiredString(context.input.target_urn, "input.target_urn", 1000));
  const requestBody = {
    actor,
    object,
    message: { text: body },
  ...
  const created = await context.dispatch(async () => {
    const response = await context.http.request(apiUrl(`/rest/socialActions/${encodedUrn(target)}/comments`), {
      method: "POST",
      headers: restApiHeaders(context, true),
  ...
  if (finalUrl !== null)
    context.setFinalUrl(finalUrl);
}
async function setReaction(context) {
  const actor = requiredString(context.input.actor, "input.actor", 500);
  requireWriteActor(context, actor, {
    member: "w_member_social_feed",
    organization: "w_organization_social_feed"
  });
  const target = socialTargetUrn(requiredString(context.input.target_urn, "input.target_urn", 1000));
  if (typeof context.input.enabled !== "boolean")
    throw new Error("input.enabled must be a boolean");
  ...
  if (enabled) {
    ...
    if (!reactionTypes.has(reaction))
      throw new Error("input.reaction is not a supported LinkedIn reaction type");
  ...
  const result = await context.dispatch(async () => {
    if (enabled) {
      if (reaction === null)
        throw new Error("input.reaction is required when input.enabled is true");
      ...
      const root = socialTargetUrn(responseString(body.root, "created reaction root", 1000));
      if (root !== target)
        throw new Error("LinkedIn created reaction root disagreed with the requested target");
      ...
      const id = responseString(body.id, "created reaction ID", 2000);
      ...
      if (headerId !== null && responseString(headerId, "created reaction header ID", 2000) !== id) {
...
async function publishPost(context) {
  ...
  const created = await context.dispatch(async () => {
    ...
    if (auxiliary.article !== null) {
      ...
      for (const item of media.items)
        ids.push(await initializeAndUploadImage(context, author, item.bytes));
      mediaIds = ids;
      if (ids.length === 1) {
        const id2 = ids[0];
        if (id2 === undefined)
          throw new Error("LinkedIn image upload omitted its media URN");
        content = {
          media: {
            id: id2,
      ...
      const id2 = await initializeAndUploadDocument(context, author, media.bytes);
      mediaIds = [id2];
      content = { media: { title: media.title, id: id2 } };
    } else if (media.kind === "video") {
      const id2 = await initializeUploadAndFinalizeVideo(context, author, media.prepared);
      mediaIds = [id2];
      content = { media: { id: id2, ...media.title === null ? {} : { title: media.title } } };
    }
    const requestBody = {
      author,
      commentary,
  ...
  }, created.mediaKind === "video" ? ["ghostget validates the reviewed MP4 bytes, type, and size locally; LinkedIn validates its 3-second to 30-minute duration and supported codecs while processing it to AVAILABLE."] : []));
  context.setFinalUrl(url);
}
async function executeLinkedInProvider(context) {
  if (context.recipe.action === "contacts.list")
    await listConnections(context);
  else if (context.recipe.action === "posts.read")
    await readPosts(context);
  else if (context.recipe.action === "posts.publish")
    await publishPost(context);
  else if (context.recipe.action === "posts.repost")
    await resharePost(context);
  else if (context.recipe.action === "comments.read")
    await readComments(context);
  else if (context.recipe.action === "comments.create")
    await createComment(context, false);
  else if (context.recipe.action === "replies.create")
    await createComment(context, true);
  else if (context.recipe.action === "reactions.set")
    await setReaction(context);
  else
    throw new Error(`official LinkedIn provider does not implement ${context.recipe.action}`);
}
export {
  executeLinkedInProvider
};
