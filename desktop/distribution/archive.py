...
def safe_name(name, signed):
    ...
    if parts[0] == "__MACOSX":
        need(signed and (len(parts) == 1 or parts[1] == "Ghostget.app" or parts[1] == "._Ghostget.app"))
    else:
        need(parts[0] == "Ghostget.app" and all(not p.startswith("._") for p in parts))
    return parts
...
def inspect(path, signed=False):
    ...
    with open(path, "rb") as raw:
        ...
        with zipfile.ZipFile(raw) as archive:
            ...
            need(offset == central_offset and names.get("Ghostget.app") is True)
            for name, directory in names.items():
                ...
                if parts[0] == "__MACOSX" and not directory:
                    ...
                    need(target in names and target.startswith("Ghostget.app"))
    after = os.lstat(path)
...
def pack(app, destination):
    ...
    need(app.name == "Ghostget.app" and not app.is_symlink() and app.is_dir())
