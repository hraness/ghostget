...
### Generated pages and status notes

- The WebMCP provider pages are generated from third-party registry data. Every interpolated count agrees with its noun (test zero, one, and several), third-party text is clipped at a word boundary and ends with “…”, and a page promises use (“Use X with your agent”) only when the registry lists a tool the agent can call.
- A review note states which Ghostget release a page describes. It does not say the page “was checked” unless a review record exists.
- `website/AGENTS.md` lists the internal words that most often reach ghostget.com and what to write instead.
