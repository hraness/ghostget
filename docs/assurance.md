...
The register holds 232 claims: 152 evidenced, 61 planned, and 19 not verified. It maps 86 guidelines from 5 guides; 66 list claims and 20 are exempt.
...
| Lean proof with differential test | 3 | 5 | 0 |
...
- Not verified:
  ...
  - The transition's own guards do not preserve the invariant alone: an accepted part whose provider message ID repeats one in the accepted prefix is rejected only by the final parseRun. The proof covers the production shape, which includes that final parse.
...
- Not verified:
  ...
  - The transition's own guards do not preserve the invariant alone: an explicit no-op success before the confirmation is consumed and a duplicate successor naming its own run are rejected only by the final parseRunJournal. The proof covers the production shape, which includes that final parse.
...
- Not verified:
  ...
  - The differential test runs the unchanged web-policy.ts over an in-memory private state store, not the real store and its helper. It samples two origins, three path segments, and four query keys, so it checks only its generated cases.
