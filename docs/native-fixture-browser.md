...
PR CI provisions only the shard that contains `src/derive.test.ts`; the existing full package-stage and release checks provision before their unchanged aggregate command. All steps are read-only with respect to package publication and providers.
