...
Run `ghostget operator doctor`, select the auth locator, and start with response content disabled:

```sh
ghostget derive start linkedin-web https://www.linkedin.com/feed/ \
  --auth linkedin-main \
  --content none \
...
```sh
ghostget derive start example-web https://example.com/feed/ \
  --auth example-main \
  --content text \
...
```sh
ghostget derive browser <id> -- upload @e5 fixture:1
```
...
```sh
ghostget derive browser <id> -- upload @single-file-input fixture:1
```
...
```bash
ghostget derive browser <id> -- choose-upload @e5 fixture:1
```
...
```sh
ghostget derive browser <id> -- upload @single-image-input fixture:1
```
...
```sh
ghostget derive browser <id> -- upload-and-seal @single-file-input fixture:1
```
...
Save the derivation ID. Use `ghostget derive list` to recover it after an interruption. ghostget serializes every lifecycle command for that ID from preflight through cleanup. A session becomes executable only after its create-only readiness marker binds the final session metadata; interrupted initialization remains visible as `ready: false`, `recoverable: true`. `socketAvailable` distinguishes a live helper from state left after a reboot.
...
```sh
ghostget derive browser <id> -- cleartext @e5
```
...
`--cookie-source`. ghostget clones that profile into the task-private derivation
...
attach its normal source user-data root to a task-private launch, so ghostget
...
Fully quit the source Chromium browser first; ghostget rejects an active or stale
...
```sh
ghostget derive review <id> --limit 50 --json
ghostget derive review <id> --entry 0 --json
printf '%s' '{"target":"known-fixture-value"}' \
  | ghostget derive review <id> --entry 0 --fixtures - --json
printf '%s' '["viewerName","followerCount"]' \
  | ghostget derive review <id> --entry 0 --field-names - --json
```
...
```sh
ghostget derive start example-web https://www.example.com/feed/ \
  --domains 'www.example.com,upload.example.net' ...
ghostget derive review <id> \
...
```sh
ghostget derive finish <id> --output /absolute/private/linkedin-web-capture
```
...
```sh
ghostget derive discard <id> --yes
```
...
`ghostget derive analyze <har> ...` can inspect an external HAR, but it cannot attest to or delete that file. Store it privately and delete it immediately after review.
